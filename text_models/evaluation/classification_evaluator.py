...
class AssignmentEvaluator(SentenceEvaluator):
    ...
    def __init__(self, dataset: Dataset, num_labels: int,
        ...
        self.csv_headers = ["epoch", "steps"] + [f"acc_at_{k}" for k in acc_at_k] \
                           + [f"precision_at_{k}" for k in precision_at_k] + [f"w_precision_at_{k}" for k in precision_at_k] \
                           + [f"recall_at_{k}" for k in recall_at_k] + [f"w_recall_at_{k}" for k in recall_at_k] \
                           + [f"f1_at_{k}" for k in f1_at_k] + [f"w_f1_at_{k}" for k in f1_at_k]
    ...
    def compute_metrics(self, model) -> Dict[str, List[float]]:
        ...
        for step, batch in enumerate(dataloader):
            ...
            preds.append(prediction.to(model.device))
        ...
        metrics = {
            AssigneeMetrics.ACC: [multiclass_accuracy(preds, labels, self.num_labels, "weighted", k).item() for k in self.acc_at_k],
            AssigneeMetrics.PRECISION: [multiclass_precision(preds, labels, self.num_labels, top_k=k).item() for k in self.precision_at_k],
            AssigneeMetrics.W_PRECISION: [multiclass_precision(preds, labels, self.num_labels, average="weighted", top_k=k).item() for k in self.precision_at_k],
            AssigneeMetrics.RECALL: [multiclass_recall(preds, labels, self.num_labels, top_k=k).item() for k in self.recall_at_k],
            AssigneeMetrics.F1: [multiclass_f1_score(preds, labels, self.num_labels, top_k=k).item() for k in self.f1_at_k],
            AssigneeMetrics.WAF1: [multiclass_f1_score(preds, labels, self.num_labels, average="weighted", top_k=k).item() for k in self.f1_at_k]
        }
